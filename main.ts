...
function doPost(e: GoogleAppsScript.Events.DoPost): GoogleAppsScript.Content.TextOutput {
    var json = JSON.parse((e.postData as any).getDataAsString());
...
function processTask(channel: string, ts: string): void {
    ...
    var url = "https://slack.com/api/conversations.replies?channel=" + channel + "&ts=" + ts + "&limit=1&inclusive=true";
    ...
    try {
        ...
        if (data.ok && data.messages.length > 0) {
            ...
            var link = "https://slack.com/archives/" + channel + "/p" + ts.replace('.', '');

            // ★ここでNotionへ保存する関数を呼び出す
            var isSuccess = addTaskToNotion(text, link);
...
function addTaskToNotion(text: string, link: string): boolean {
    ...
    var post_url = "https://api.notion.com/v1/pages";
 ...
 * @returns {void}
 */
function addSlackReaction(channel: string, ts: string, reactionName: string): void {
